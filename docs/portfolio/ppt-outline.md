...
수동 EXPLAIN에서는 covering index가 `Index Only Scan`으로 동작했지만, API 경로에서는 DB 집계 조회가 느렸다. Repository 코드를 다시 보는 데서 멈추지 않고 Hibernate SQL 로그와 바인딩 파라미터를 확인해, JPQL `count(p)`가 실제 SQL에서 `count(p.id)`로 변환되는 차이를 발견했다.

포함 내용:

- 수동 SQL의 `count(*)`는 `id` 컬럼을 읽지 않아 `Index Only Scan` 가능
- API SQL의 `count(p.id)`는 covering index에 없는 `id`를 요구해 heap 접근 가능
- covering index는 쿼리에 필요한 모든 컬럼이 인덱스에 있을 때 효과가 큼
- JPQL과 실제 SQL은 다를 수 있으므로 성능 분석에서는 SQL 로그와 실행계획 확인이 필수
- native SQL은 `count(*)`를 명시할 수 있지만, `select new DTO(...)` 대신 interface projection 같은 매핑 방식이 필요

발표 문장:

처음에는 같은 GROUP BY 집계라고 생각했지만, 수동 EXPLAIN과 API 실행 시간이 크게 달랐습니다. 그래서 API 내부 구간을 나누고 Hibernate가 실제로 만든 SQL을 확인했습니다. 그 결과 JPQL의 `count(p)`가 SQL에서는 `count(p.id)`로 바뀌었고, 제가 만든 covering index에 `id`가 없어서 API 경로에서는 테이블 본문 접근이 발생할 수 있다는 점을 알게 됐습니다. 이 경험을 통해 ORM을 쓸 때도 최종 성능 판단은 Repository 코드가 아니라 실제 SQL과 EXPLAIN으로 해야 한다는 점을 배웠습니다.

다음 실험:

native query로 `count(*)`를 명시하고 interface projection으로 결과를 받은 뒤, API 실제 SQL과 EXPLAIN에서 `Index Only Scan` 여부를 다시 확인한다. 이후 GROUP_BY_QUERY와 GROUP_BY_BULK_SAVE API 시간을 같은 조건에서 3회 평균으로 재측정한다.

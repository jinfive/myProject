...
현재는 1단계인 **정산 배치 성능 비교용 MVP 중 GROUP_BY_BULK_SAVE 1차 구현 완료** 상태이다.
...
- 더미 데이터 자동 생성
    ...
    - 결제 데이터 100,000건
    - 특정일 거래 약 70,000건
- benchmark-medium 로컬 데이터셋 재생성
    - 가맹점 5,000개
    - 결제 데이터 1,000,000건
    - 실행일 기준 정산 가능 데이터 1,000,000건
...
- GROUP_BY_BULK_SAVE 1차 구현도 같은 GROUP BY 집계 결과를 재사용하며 Payment 전체 Entity List를 조회하지 않는다.
...
GROUP_BY_QUERY와 GROUP_BY_BULK_SAVE에서 사용하는 집계 쿼리의 개념은 다음과 같다. 실제 코드는 JPQL을 사용하며 Entity 필드명인 `transactionDate`, `status`, `type`, `amount`, `merchant`를 기준으로 작성한다.
...
GROUP_BY_BULK_SAVE 1차 구현은 위 집계 결과로 생성한 Settlement 목록을 `saveAll`로 저장한다. 이 단계에서는 Hibernate `batch_size`나 PostgreSQL JDBC `reWriteBatchedInserts=true`를 적용하지 않았으므로 실제 DB batch insert 효과는 다음 단계에서 별도 검토한다.

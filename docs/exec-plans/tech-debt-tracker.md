...
현재는 1단계인 **정산 배치 성능 비교용 MVP 중 10만 건 기준 실험 완료 및 확장 실험 기획** 단계이다.
...
1. DB 설정 불일치
2. 핵심 정산 로직 테스트 부족
3. 100만 건 / Merchant 5,000개 중간 확장 실험 미완료
4. 1000만 건 / Merchant 5,000~10,000개 최종 대용량 실험 미완료
5. 인덱스 적용 및 성능 비교 미구현
6. EXPLAIN ANALYZE 실행 계획 확인 미완료
7. 대사 기능 미구현
8. RUNNING 중복 실행 방지 미구현
9. 날짜 파티셔닝 고도화 문서화 필요
10. 실제 코드와 DB 문서 불일치
11. 성능 비교 결과 문서화 부족
```
...
다음 확장 실험에서는 데이터 규모를 설정 기반으로 분리하는 방향을 검토한다. 10만 건은 `benchmark-small`, 100만 건 / Merchant 5,000개는 `benchmark-medium`, 1000만 건 / Merchant 10,000개는 `benchmark-large`처럼 구분하되, 기존 `payments`와 `merchants`를 무단 삭제하지 않고 명확한 설정값이나 사용자 확인을 기준으로 재생성한다.
...
1000만 건 실험 이후 인덱스를 추가하더라도 실제 집계 쿼리가 인덱스를 사용하는지 확인하지 않으면 성능 개선 근거가 약하다.

### 해결 방향

1000만 건에서 GROUP_BY_QUERY가 수 초 이상 걸리거나 `payments` 전체 스캔 비용이 크다고 확인된 뒤 인덱스 적용 전후 `EXPLAIN ANALYZE` 결과를 기록한다. Seq Scan/Index Scan 여부, 실행 시간, rows 예측과 실제 rows 차이를 README와 PPT 재료로 남긴다. 대량 데이터 생성 또는 날짜 동기화 후에는 필요 시 `ANALYZE payments;`를 실행하도록 문서화한다.
...
현재 10만 건 / Merchant 100개 실험에서는 Settlement 저장 결과가 100건 수준이라 batch 설정 효과를 판단하기 어렵다. Payment 수가 아니라 실제 insert되는 Settlement 수가 저장 최적화 효과에 영향을 준다.
...
100만 건 / Merchant 5,000개와 1000만 건 / Merchant 5,000~10,000개 실험에서 Settlement 저장 건수가 늘어난 뒤 저장 병목을 확인한다.

saveAll만으로 저장 성능 개선이 제한적이면 다음 설정을 별도 단계에서 적용하고 성능과 정합성을 재검증한다.
...
`reWriteBatchedInserts`는 PostgreSQL 서버 설정이 아니라 PostgreSQL JDBC 드라이버 옵션이다.

### 완료 기준

* [ ] Settlement 저장 건수가 5,000건 이상인 조건에서 saveAll 기준 실행 시간이 기록되었다.
* [ ] Hibernate batch_size 적용 전후 실행 시간이 비교되었다.
* [ ] PostgreSQL reWriteBatchedInserts 적용 여부와 효과가 분리 기록되었다.
* [ ] BASIC_LOOP, GROUP_BY_QUERY, GROUP_BY_BULK_SAVE 결과 금액 동일성이 재검증되었다.
* [ ] README와 포트폴리오 문서에 실제 측정값만 반영되었다.

---

## TD-027: 100만/1000만 건 정산 배치 성능 확장 실험 미완료
...
10만 건 / Merchant 100개 기준에서는 BASIC_LOOP 882ms, GROUP_BY_QUERY 133ms, GROUP_BY_BULK_SAVE 110ms를 확인했다. 이 결과로 Payment 전체 조회 방식의 비효율성과 DB GROUP BY 집계 개선 효과는 확인했다.

하지만 Merchant 수가 100개라 Settlement 저장 결과도 100건 수준에 그쳤고, saveAll 기반 저장 최적화 효과나 인덱스 필요성을 판단하기에는 데이터 규모가 작다.
...
정산 조건에 자주 사용되는 컬럼에 대한 인덱스 적용과 성능 비교가 아직 구현되어 있지 않다. 다만 현재 10만 건 기준에서는 GROUP_BY_QUERY만으로 충분한 개선을 확인했으므로 인덱스를 즉시 적용하지 않는다.

1000만 건에서 GROUP_BY_QUERY가 수 초 이상 걸리거나 `EXPLAIN ANALYZE`에서 `payments` 전체 스캔 비용이 크다고 확인되면 다음 인덱스를 우선 검토한다.
...
100만/1000만 건 실험으로 조회 병목을 확인한 뒤 인덱스 적용 전후를 비교한다.

권장 구현 방향:

```txt
1. 현재 쿼리 조건 확인
2. 1000만 건에서 인덱스 적용 전 실행 시간과 EXPLAIN ANALYZE 기록
3. Payment에 필요한 인덱스 추가
...
```txt
1. TD-027 100만/1000만 건 정산 배치 성능 확장 실험
2. TD-025 Hibernate batch_size와 reWriteBatchedInserts 적용 검토
3. TD-006 1000만 건 병목 확인 후 GROUP_BY_BULK_INDEX 구현 및 인덱스 적용 비교
4. TD-021 EXPLAIN ANALYZE 실행 계획 확인
5. TD-022 Payment 원천 데이터와 Settlement 결과 대사 기능 구현
6. TD-023 RUNNING 상태 기준 중복 실행 방지
7. TD-024 날짜 파티셔닝 고도화 항목 문서화
8. TD-001 DB 설정 불일치 해결
9. TD-007 DB 문서 갱신
10. TD-009 성능 비교 결과 문서화
11. TD-010 정산 처리 전략 구조 분리
12. TD-011 프론트엔드 구조 정리
13. TD-012 공통 예외 응답 구조 정리
14. TD-013~TD-018 2단계 증권 실시간 처리 설계
```

...
2. 정산 계산 테스트 추가
3. 중복 정산 방지 테스트 추가
4. GROUP BY 기반 정산 개선 전략 추가
5. 벌크 저장 개선 전략 추가
6. 인덱스 적용 및 성능 비교
7. 배치 실패 이력 저장
8. 대사 기능 추가
9. 성능 비교 화면 확장
10. docs/generated/db-schema.md 갱신
11. README 성능 개선 결과 정리
12. PPT 제작용 포트폴리오 요약 문서 작성
```

현재 가장 중요한 작업은 BASIC_LOOP에서 끝내지 않고, 성능 개선 전략을 추가해 포트폴리오의 핵심 메시지를 완성하는 것이다.

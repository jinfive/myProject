...
`GROUP_BY_BULK_SAVE`의 110ms는 2026-05-14 21:21:54에 같은 로컬 DB와 같은 Payment 100,000건으로 측정한 1회 결과입니다. 이번 단계는 `saveAll`만 적용한 1차 구현이므로 실제 DB batch insert 효과를 단정하지 않고, Hibernate `batch_size`와 PostgreSQL JDBC `reWriteBatchedInserts=true`는 후속 단계에서 분리 측정합니다.
...
- GROUP_BY_BULK_SAVE는 GROUP BY 집계는 유지하고 Settlement 저장 방식만 `saveAll` 기반으로 개선하는 단계입니다.
- Hibernate batch_size와 PostgreSQL reWriteBatchedInserts는 saveAll 적용 결과를 확인한 뒤 별도 단계로 분리해 측정합니다.

...
`GROUP_BY_QUERY`에서는 `payments` 전체 Entity 목록을 조회하지 않고, DB GROUP BY로 가맹점별 결제금액과 취소금액, 완료 처리 건수를 먼저 집계합니다. Java에서는 집계 결과만 받아 BASIC_LOOP와 같은 반올림 정책으로 수수료와 최종 정산금액을 계산합니다. 이번 단계의 개선 효과를 분리해서 보기 위해 Settlement 저장은 BASIC_LOOP와 동일하게 개별 저장으로 유지했고, 벌크 저장과 인덱스 적용은 다음 단계로 남겼습니다.
...
`http://localhost:5173`에서 정산일자 `2026-05-08`, 처리 전략 `BASIC_LOOP` 또는 `GROUP_BY_QUERY`를 선택한 뒤 정산 배치 실행 버튼을 누르면 백엔드 API를 호출하고 정산 결과와 배치 실행 이력을 갱신합니다. `GROUP_BY_BULK_SAVE`, `GROUP_BY_BULK_INDEX`는 아직 구현하지 않았으므로 프론트 선택 목록에서 비활성화했습니다.
...
- GROUP_BY_QUERY는 Payment 전체 조회를 제거하고 DB GROUP BY 집계로 가져오는 데이터량을 줄이는 단계입니다.
- 결과 동일성 검증은 성능 개선보다 정합성이 먼저라는 원칙을 확인하는 단계입니다.
- GROUP_BY_BULK_SAVE는 GROUP BY 집계는 유지하고 Settlement 저장 방식만 일괄 저장으로 개선하는 단계입니다.
- GROUP_BY_BULK_INDEX는 조회 조건과 GROUP BY 조건을 기준으로 필요한 인덱스를 판단하고 적용하는 단계입니다.

...
100만 건에서 `BASIC_LOOP`가 너무 오래 걸리거나 메모리 부담이 크면 실행을 중단하고, 전체 Payment를 애플리케이션으로 로딩하는 방식의 한계로 기록합니다. 1000만 건에서는 `BASIC_LOOP`를 필수 전략으로 보지 않고, DB GROUP BY 기반 전략 중심으로 정합성과 실행 시간을 비교합니다.
...
100만 건과 1000만 건 실험을 구현할 때는 데이터 규모를 코드에 고정하지 않고 설정이나 프로파일로 분리하는 방향을 검토합니다.
...
예상 프로파일은 `benchmark-small`(100,000 payments / 100 merchants), `benchmark-medium`(1,000,000 payments / 5,000 merchants), `benchmark-large`(10,000,000 payments / 10,000 merchants)입니다. 기존 `payments`와 `merchants`는 무단 삭제하지 않고, 재생성이 필요하면 명확한 설정값이나 사용자 확인을 기준으로 처리합니다. `batch_job_histories`는 실행 이력이므로 계속 보존합니다.
